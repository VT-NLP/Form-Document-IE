...
class REDecoder(nn.Module):
    ...
    def forward(self, hidden_states, entities, relations, region_ids):
        ...
        all_pred_relations = []
        for b in range(batch_size):
            head_entities = torch.tensor(relations[b]["head"], device=device)#get indices of all entity present in entities dict
            ...
            # if("region_id" in entities[b]):
            #     entities_region_ids = torch.tensor(entities[b]["region_id"], device=device)
            ...
            #             link = 7*[0]
            #             link[0] = 1
            #             link[1], link[2] = self.check_lr_tb(entities[b]["bbox"][head_entities[i]],entities[b]["bbox"][tail_entities[i]])
            #             feature_links.append(link)
            #         else:
            #             link = 7*[0]
            #             link[3], link[4] = self.check_lr_tb(entities[b]["bbox"][head_entities[i]],entities[b]["bbox"][tail_entities[i]])
            #             region_head_bbox = self.find_bbox(region_ids[b], region_head[i])
            #             region_tail_bbox = self.find_bbox(region_ids[b], region_tail[i])
