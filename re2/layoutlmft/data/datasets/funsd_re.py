...
def create_DatasetDict(folder_path):
    funsd_cache = "./layoutlmft/data/datasets/__pycache__/funsd.pickle"
    if os.path.isfile(funsd_cache):
        with open(funsd_cache, "rb") as f:
            return pickle.load(f)
    train_path = os.path.join(folder_path, 'funsd_train')
    test_path = os.path.join(folder_path, 'funsd_test')
...
def generate_examples(filepath):
    ...
    for guid, file in enumerate(sorted(os.listdir(ann_dir))):
        ...
        with open('xfun_en.json') as json_file:
        ...
        for item in data["form"]:
            ...
            if len(item['linking'])!=0:
                relation_list.append(item['linking'])
    ...
    # data_file = open(f"{ann_dir.split('/')[-2]}.json",'w')
    # json.dump(dataset, data_file, indent = 6, ensure_ascii=False)
