...
from ...modules.decoders.re_gat import REDecoder
from ...modules.eGAT.egat import GAT
...
class LayoutLMv2ForRelationExtraction(LayoutLMv2PreTrainedModel):
    ...
    def forward(
        ...
        sequence_output, image_output = outputs[0][:, :seq_length], outputs[0][:, seq_length:]
        sequence_output = self.dropout(sequence_output)
        loss, pred_relations = self.extractor(sequence_output, entities, relations, region_ids)
...
class LayoutLMv2ForRelationExtractionWithEGAT(LayoutLMv2PreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.layoutlmv2 = LayoutLMv2Model(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.extractor = REDecoder(config)
        #############################################################
        self.gat = GAT(nfeat=768, efeat=7, batch_size= 2, graph_size=256, nhid=768, dropout=0.6, alpha=0.2, nheads=8)
        #############################################################
        self.init_weights()        
    ...
    def forward(
        ...
        sequence_output, image_output = outputs[0][:, :seq_length], outputs[0][:, seq_length:]
        sequence_output = self.dropout(sequence_output)
        node_output, edge_output =  self.gat(sequence_output, region_ids, entities)
        loss, pred_relations = self.extractor(sequence_output, node_output, edge_output, entities, relations, region_ids)
